...
def prepare_dataset():
    if os.path.exists(ds_path):
        ...
        ds = ds.rename(columns={"Beer_Name": "name", "Brew_Factory_Name": "factory", "Style": "style", "ABV": "abv"})
        ds = ds[["_id", "name", "factory", "style", "abv"]]
        ds.set_index("_id")
        # print(ds.dtypes)
        # print(ds)

        # Preprocess numeric columns
        ds["abv"] = ds["abv"].str.rstrip("%")
        ds["abv"] = pd.to_numeric(ds["abv"], errors="coerce")
        print(ds.dtypes)
        print(ds)
...
def prepare_ground_truth(l_map, r_map):
    if os.path.exists(lab_path) and os.path.exists(match_path):
        ...
        lab = lab.rename(columns={"ltable.Label": "l_id", "rtable.Label": "r_id"})
        # print(lab.columns)
        # print(lab)
