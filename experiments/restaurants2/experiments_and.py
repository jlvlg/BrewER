...
def resolve(records):
    ...
    entity["name"] = (
        records["name"].mode().iloc[0] if not records["name"].mode().empty else None
    )
    entity["rating"] = records["rating"].mean()
    entity["votes"] = records["votes"].mean()
    return entity
...
batch_filtered = resolved[
    (resolved["votes"] > 50) & (resolved["rating"] > 3)
...
brewer = (
    ...
    .select(
        ...
        ("table.name", brewer.resolution_function.VOTE),
        ("table.rating", brewer.resolution_function.AVG),
        ("table.votes", brewer.resolution_function.AVG),
        order_by=("table.rating", brewer.SqlOrderBy.ASC),
    )
    .where("table.votes > 50 and table.rating > 3")
    .subscribe(listener)
)
