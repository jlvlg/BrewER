...
def resolve(records):
    ...
    entity["rating_value"] = records["rating_value"].mean()
    entity["price"] = records["price"].mean()
    entity["length"] = records["length"].mean()
    return entity
...
batch_filtered = resolved[
    (resolved["price"] > 10) & (resolved["length"] > 40)
    ...
    brewer_results.append(
        {
            ...
            "order": entity["table_rating_value"],
...
brewer = (
    ...
    .select(
        ...
        ("table.rating_value", brewer.resolution_function.AVG),
        ("table.price", brewer.resolution_function.AVG),
        ("table.length", brewer.resolution_function.AVG),
        order_by=("table.rating_value", brewer.SqlOrderBy.ASC),
    )
    .where("table.price > 10 and table.length > 40")
    .subscribe(listener)
)
